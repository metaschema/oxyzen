...
scan:function(next){f$.db.afterscan=next;
	...
	if(f$.user){scanurl+='&access_token='+f$.user.token}
...
update:function(doc,log,_first,_skipreindex){var _this=this;var k=doc.$key;if(!_skipreindex){this._doindex(doc,k,this.docnamefield);}delete doc.$key;if(!log){log='Object Modified'}
...
find:function(s,next,nextrem,_collections){if(!_collections){_collections=this.collections}
	if(s=='all'){for(var c=0;c<_collections.length;c++){this.getall(_collections[c],next,nextrem)}}
		...
		for(var c=0;c<ee.length;c++){cn=ee[c].replace('-','/');
			firebase.database().ref(cn+'/rels').once('value',function(snap){var RR=snap.val();var R;for(R in RR){
		...
		for(var c=0;c<_collections.length;c++){cn=_collections[c];
			...
			tr.on('child_added',fn(cn));tr.on('child_changed',fn(cn));tr.on('child_removed',fn2);
	}}else{var _this=this;var popped=[];var xx=s.split(' ');var xlen=xx.length;s=s.toLowerCase();
		s=s.replace(/\n|\t|\r|{|}|\||<|>|\\|!|"|£|$|%|&|\/|\(|\)|=|\?|'|"|^|\*|\+|\[|\]|§|°|@|\.|,|;|:/g,' ');
